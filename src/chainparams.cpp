...
//		pchMessageStart[3] = G_CONFIG_TABLE::Message_mainNet[3];
		...
		assert(CreateGenesisRewardTx(genesis.vptx, IniCfg().GetIntPubKey(MAIN_NET)));
...
//        pchMessageStart[3] = G_CONFIG_TABLE::Message_testNet[3];
		...
		genesis.SetNonce(99);
		genesis.vptx.clear();
		assert(CreateGenesisRewardTx(genesis.vptx,IniCfg().GetIntPubKey(TEST_NET)));
		genesis.SetHashMerkleRoot(genesis.BuildMerkleTree());
		hashGenesisBlock = genesis.GetHash();
...
public:
	CRegTestParams() {
		memcpy(pchMessageStart,IniCfg().GetMagicNumber(RETTEST_NET),sizeof(pchMessageStart));
...
//		pchMessageStart[3] = G_CONFIG_TABLE::Message_regTest[3];
		nSubsidyHalvingInterval =  IniCfg().GetHalvingInterval(RETTEST_NET);
		bnProofOfStakeLimit = ~arith_uint256(0) >> 6;     //target:00000011 11111111 11111111
		genesis.SetTime(IniCfg().GetStartTimeInit(RETTEST_NET));
		genesis.SetBits(0x2003ffff);
		genesis.SetNonce(68);
		genesis.vptx.clear();
		assert(CreateGenesisRewardTx(genesis.vptx, IniCfg().GetIntPubKey(RETTEST_NET)));
		genesis.SetHashMerkleRoot(genesis.BuildMerkleTree());
		hashGenesisBlock = genesis.GetHash();
		nDefaultPort = IniCfg().GetnDefaultPort(RETTEST_NET) ;
		nTargetSpacing = 20;
...
//		}
		assert(hashGenesisBlock == IniCfg().GetIntHash(RETTEST_NET));
